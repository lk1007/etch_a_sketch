...
class Node:
    ...
    def __init__(self, x: int, y: int) -> None:
        ...
        Args:
            x,y (int): Position of node in image
        Returns:

        """
        self.pos: tuple[int, int] = x, y

    def dist(self, destination: Node) -> float:
        """Finds distance between self and destination

        Args:
            destination(Node) to calculate distance to  
        Returns:
            distance(float) between self and destination
        """

        calculated_distance: float = math.dist(self.pos, destination.pos)
    ...
    def cost(self, destination: Node) -> float:
        """Finds cost of going from self to destination

        Args:
            destination(Node) to calculate cost function of  
        Returns:
            cost(float) of going from self to destination
        """
...
def to_nodes(arr: list[list[int]]) -> dict[tuple[int, int], Node]:
    """Converts pixel array to nodes

    Args:
        pixel array (int array)
    Returns:
        dictionary of 
    """
    def get_neighbors(i: int, j: int) -> list[int]:
        ...
        if (j < 1 or j >= len(arr)-1):
            return None
        for ii, row in enumerate(arr[i-1:i+2]):
            for jj, elt in (row[j-1:j+2]):
                if (ii == jj):
                    return None
                else:
                    neighbors.append(elt)

    if usebar:
        task0: progress.TaskID = progress.add_task(
            "Converting " + str(len(arr[0])) + " x " + str(len(arr)) + " image...", total=len(arr))
    ...
    for i, row in enumerate(arr):
        ...
        for j, elt in enumerate(row):
            ...
            isolated = False
            try:
                if (reduce(lambda acc, x: acc and x <= black), get_neighbors(i, j)):
                    landlocked = True
                if (reduce(lambda acc, x: acc and x >= black), get_neighbors(i, j)):
                    isolated = True
            except:
...
def extend_path(path_entry, node: Node, updatepath=True):
    newlist = []
...
def adjacent_nodes(node_dict, node, directly=True, unex=False, lastnode=None):
    ...
    if next_in_direction is not None:
        pos = [next_in_direction, (x, y - 1), (x + 1, y), (x - 1, y), (x, y + 1), (x + 1, y + 1), (x - 1, y - 1),
               (x - 1, y + 1), (x + 1, y - 1)
               ]
    else:
        pos = [(x, y - 1), (x + 1, y), (x - 1, y), (x, y + 1), (x + 1, y + 1), (x - 1, y - 1),
               (x - 1, y + 1), (x + 1, y - 1)
               ]
    for p in pos:
        if p in node_dict:
    ...
    if usebar:
        if end is None:
            task2 = progress.add_task(
                "Computing " + str(len(node_dict)) + " nodes...", total=len(node_dict))
        else:
            task2 = progress.add_task(
                "Finding node from " + str(len(node_dict)) + " nodes...", total=len(node_dict))
...
def explore(node_dict, precision_factor=1000, counter=None):
    ...
    if usebar:
        task1 = progress.add_task(
            "Tracing " + str(len(node_dict)) + " nodes...", total=len(node_dict), )
    ...
    while explored_count < len(node_dict):
        ...
        while current is not None:
            ...
            if usebar:
                progress.update(
                    task1, advance=1, description="Tracing {}/{} nodes...".format(explored_count, len(node_dict)))
            before = last
            ...
            for node in adjacent_nodes(node_dict, current, directly=False):
                if not node.f and not node.e:
                    distances[node] = extend_path(
                        distances[current], node, updatepath=False)
                    node.f = True
            ...
            adj = adjacent_nodes(node_dict, current,
                                 directly=False, unex=True, lastnode=before)
            if len(adj) == 0:
                nextnode = current
                if usebar:
                    backtracking = progress.add_task(
                        "Backtracking...", total=None)
                ...
                if nextnode is not None:
                    nextnode = adjacent_nodes(
                        node_dict, nextnode, directly=True, unex=True)[0]
        ...
        if explored_count < len(node_dict):
            ...
            distances[nextnode] = extend_path(
                distances[last], nextnode, updatepath=False)  # frompath
...
def path(name, fname, s, precision_factor=100000000000000, brightness=200, counter=None):
    with progress:
        ...
        if usebar:
            writefile = progress.add_task(
                "Writing sequence of {}...".format(len(e)), total=len(e))
...
class PathMaker():
    def __init__(self, dest: str, img: str, dim: tuple[int, int] = (320, 240), precision_factor: int = 200000):
        self.dest: str = dest
        self.img: str = img
        self.dim: tuple[int, int] = dim
        self.pf: int = precision_factor
        self.processes = {}
        self.files = {}
        self.counter = Counter()

    def pathmaker(self, brightness: int):
        def f():
            path(self.dest + str(brightness), self.img, self.dim,
                 precision_factor=self.pf, brightness=brightness, counter=self.counter)
        return f
